...
export default function () {
    store.registerHook('element.initialized', (el, component) => {
        ...
        el.__wire_polling_interval = intervalId
    })

    store.registerHook('element.updating', (from, to, component) => {
        if (from.__wire_polling_interval !== undefined) return
        ...
        if (wireDirectives(from).missing('poll') && wireDirectives(to).has('poll')) {
            setTimeout(() => {
                ...
                from.__wire_polling_interval = intervalId
            }, 0)
        }
    })
}
...
function fireActionOnInterval(node, component) {
    ...
    return setInterval(() => {
        ...
        if (store.wireIsInBackground && ! directive.modifiers.includes('keep-alive')) {
        ...
        // Don't poll if wire is offline as well.
        if (store.wireIsOffline) return

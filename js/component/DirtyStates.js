...
export default function () {
    ...
    store.registerHook('message.received', (message, component) => {
        component.dirtyEls.forEach(element => {
            if (element.__wire_dirty_cleanup) {
                element.__wire_dirty_cleanup.forEach(cleanup => cleanup())
                delete element.__wire_dirty_cleanup
            }
        })
    })
        ...
        .forEach(directive => {
            if (directive.modifiers.includes('class')) {
                ...
                if (directive.modifiers.includes('remove') !== isDirty) {
                    ...
                    el.__wire_dirty_cleanup.push(() => el.classList.remove(...classes))
                } else {
                    el.classList.remove(...classes)
                    el.__wire_dirty_cleanup.push(() => el.classList.add(...classes))
                }
            } else if (directive.modifiers.includes('attr')) {
                if (directive.modifiers.includes('remove') !== isDirty) {
                    el.setAttribute(directive.value, true)
                    el.__wire_dirty_cleanup.push(() => el.removeAttribute(directive.value))
                } else {
                    el.removeAttribute(directive.value)
                    el.__wire_dirty_cleanup.push(() => el.setAttribute(directive.value, true))
                }
            } else if (!wireDirectives(el).get('model')) {
                el.style.display = isDirty ? 'inline-block' : 'none'
                el.__wire_dirty_cleanup.push(() => (el.style.display = isDirty ? 'none' : 'inline-block'))
            }
        });
}

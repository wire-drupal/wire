...
export default function () {
    ...
    store.registerHook('component.initialized', component => {
        ...
        setTimeout(() => {
            ...
            WireStateManager.replaceState(url, response, component)
    ...
    store.registerHook('message.processed', (message, component) => {
        ...
        if ('path' in effects && effects.path !== window.location.href) {
            ...
            WireStateManager.pushState(url, response, component)
            ...
            if (componentIdsThatAreWritingToHistoryState.has(component.id)) {
                WireStateManager.replaceState(window.location.href, response, component)
            }
        }
    })

    window.addEventListener('popstate', event => {
        if (WireStateManager.missingState(event)) return

        WireStateManager.replayResponses(event, (response, component) => {
    ...
    function normalizeResponse(response, component) {
        ...
        // Sometimes Wire doesn't return html from the server to save on bandwidth.
...
let WireStateManager = {
    ...
    updateState(method, url, response, component) {
        ...
        let fullstateObject = Object.assign(history.state || {}, { wire: stateArray })
    ...
    replayResponses(event, callback) {
        if (! event.state.wire) return

        let state = typeof event.state.wire === 'string'
            ? new WireState(this.getFromSession(event.state.wire))
            : new WireState(event.state.wire)

        state.replayResponses(callback)
    },

    currentState() {
        if (! history.state) return new WireState
        if (! history.state.wire) return new WireState

        let state = typeof history.state.wire === 'string'
            ? new WireState(this.getFromSession(history.state.wire))
            : new WireState(history.state.wire)

        return state
    },

    missingState(event) {
        return ! (event.state && event.state.wire)
    },

    clearState() {
        // This is to prevent exponentially increasing the size of our state on page refresh.
        if (window.history.state) window.history.state.wire = (new WireState).toStateArray();
    },

    storeInSession(value) {
        let key = 'wire:'+(new Date).getTime()
    ...
    tryToStoreInSession(key, value) {
        ...
        try {
            ...
            let oldestTimestamp = Object.keys(sessionStorage)
                .map(key => Number(key.replace('wire:', '')))
                .sort()
                .shift()

            if (! oldestTimestamp) return

            sessionStorage.removeItem('wire:'+oldestTimestamp)
...
class WireState
{
    ...
    findComponentBySignature(signature) {
        ...
        return sameNamedComponents[componentIndex] || sameNamedComponents[0] || console.warn(`Wire: couldn't find component on page: ${componentName}`)
    }

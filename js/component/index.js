...
export default class Component {
    ...
    checkForMultipleRootElements() {
        ...
        if (countElementsBeforeMarker(this.el.nextSibling) > 0) {
            console.warn(`Wire: Multiple root elements detected. This is not supported. See docs for more information https://wire-drupal.com/docs/troubleshooting#root-element-issues`, this.el)
        }
    }
    ...
    updateServerMemoFromResponseAndMergeBackIntoResponse(message) {
        ...
        // because browsers mess with the object key order which will break Wire request checksum checks.
        ...
        Object.entries(message.response.serverMemo).forEach(([key, value]) => {
            ...
            if (key === 'data') {
                Object.entries(value || {}).forEach(([dataKey, dataValue]) => {
                    ...
                    // Because Wire (for payload reduction purposes) only returns the data that has changed,
    ...
    handleMorph(dom) {
        ...
        morphdom(this.el, dom, {
            ...
            onNodeDiscarded: node => {
                ...
                if (node.__wire) {
                    store.removeComponent(node.__wire)
                }
            ...
            onBeforeElUpdated: (from, to) => {
                ...
                // Honor the "wire:ignore" attribute or the .__wire_ignore element property.
                if (
                    fromDirectives.has('ignore') ||
                    from.__wire_ignore === true ||
                    from.__wire_ignore_self === true
                ) {
                    if (
                        (fromDirectives.has('ignore') &&
                            fromDirectives
                ...
                // Give the root Wire "to" element, the same object reference as the "from"
                // element. This ensures new Alpine magics like $wire and @entangle can
                // initialize in the context of a real Wire component object.
                if (DOM.isComponentRootEl(from)) to.__wire = this
    ...
    get $wire() {
        ...
        return (this.dollarWireProxy = new Proxy(refObj, {
            get(object, property) {
                ...
                // Forward "emits" to base Wire object.

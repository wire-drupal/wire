...
function processLoadingDirective(component, el, directive) {
    ...
    if (directives.get('target')) {
        ...
        const nonActionOrModelWireDirectives = [
            'init',
            'dirty',
            'offline',
            'target',
        ...
        actionNames = directives
            .all()
            .filter(i => !nonActionOrModelWireDirectives.includes(i.type))
            .map(i => i.method)
...
function doAndSetCallbackOnElToUndo(el, directive, doCallback, undoCallback) {
    ...
    if (directive.modifiers.includes('delay')) {
        ...
        let timeout = setTimeout(() => {
            doCallback()
            el.__wire_on_finish_loading.push(() => undoCallback())
        }, duration)

        el.__wire_on_finish_loading.push(() => clearTimeout(timeout))
    } else {
        doCallback()
        el.__wire_on_finish_loading.push(() => undoCallback())
    }
...
function endLoading(els) {
    els.forEach(({ el }) => {
        while (el.__wire_on_finish_loading.length > 0) {
            el.__wire_on_finish_loading.shift()()
        }
    })
}
